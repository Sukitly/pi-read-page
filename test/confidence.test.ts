...
function page(overrides: Partial<ExtractedPage>): ExtractedPage {
  return {
    ...
    title: "Example",
    markdown: "This is a normal short article. It has Log in in the nav but content is readable.",
    contentHtml: "<article><p>This is a normal short article.</p><a>Log in</a></article>",
    fullHtml: "",
    textLength: 80,
...
describe("confidence and handoff", () => {
  ...
  it("marks thin content as low confidence without making it user-actionable", () => {
    const extracted = withConfidence({ markdown: "tiny", contentHtml: "<p>tiny</p>", metadata: { ...page({}).metadata, wordCount: 1 } });
  ...
  it("hands off captcha pages", () => {
    const extracted = withConfidence({ markdown: "Please verify you are human", contentHtml: "<p>Please verify you are human</p>" });
    const decision = decideUserAction(extracted);
