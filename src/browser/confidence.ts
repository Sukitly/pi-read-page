import type { ConfidenceReport, ExtractedPage, HandoffReason, UserActionDecision } from "../types";

const CAPTCHA_RE = /captcha|recaptcha|hcaptcha|verify you are human|human verification|are you human|请完成安全验证|人机验证/i;
const BLOCK_RE = /cloudflare|attention required|checking your browser|just a moment|access denied|temporarily blocked|访问受限|安全检查/i;
const LOGIN_WALL_RE = /sign in to continue|log in to continue|login to continue|please sign in|please log in|subscribe to continue|sign up to continue|join to view|登录后(查看|继续|阅读)|请登录(后)?(查看|继续|阅读)?/i;
const LOGIN_URL_RE = /\/(login|signin|sign-in|auth|oauth|session)(\/|$|[?#])/i;
const NAV_ONLY_RE = /home\s+about\s+contact|privacy policy|terms of service|all rights reserved/i;

export function assessConfidence(page: Omit<ExtractedPage, "confidence">): ConfidenceReport {
  ...
  if (!page.metadata.description && !page.metadata.published && !page.metadata.author && !page.metadata.site) {
    score -= 5;
...
function detectActionableHandoff(page: ExtractedPage): { reason: HandoffReason; message: string } | undefined {
  const markdown = page.markdown.trim();
  const sample = `${page.url}\n${page.title}\n${markdown.slice(0, 10_000)}`.toLowerCase();

  if (CAPTCHA_RE.test(sample)) {
    return { reason: "captcha", message: "The page appears to require captcha or human verification." };
  }

  if (BLOCK_RE.test(sample)) {
    return { reason: "blocked", message: "The page appears to be blocked by an anti-bot or permission interstitial." };
  }

  if (isLoginWall(page, markdown.length, page.metadata.wordCount || estimateWordCount(markdown))) {
    return { reason: "login_required", message: "The page appears to require login, subscription access, or manual navigation." };
  }

  return undefined;
}

function isLoginWall(page: Pick<ExtractedPage, "url" | "title" | "markdown" | "contentHtml">, markdownLength: number, wordCount: number): boolean {
...
function estimateWordCount(markdown: string): number {
  const asciiWords = markdown.match(/[A-Za-z0-9_]+(?:[-'][A-Za-z0-9_]+)*/g)?.length || 0;
